...
from models.ulip_legacy.dataset_3d import *
import numpy as np
...
class ULIP2_WITH_OPENCLIP(nn.Module):
    ...
    def encode_pc(self, pc, return_intermediate=False, return_layers=None):
        if return_intermediate:
            concat_f, global_feat, local_feat, features, neighborhood, center, patch_idx = self.point_encoder(pc,
                                                                                                              return_intermediate,
                                                                                                              return_layers)
            pc_embed = concat_f @ self.pc_projection
            return pc_embed, global_feat, local_feat, features, neighborhood, center, patch_idx
        else:
            pc_feat = self.point_encoder(pc, return_intermediate, return_layers)                 
            pc_embed = pc_feat @ self.pc_projection
            return pc_embed
...
def ULIP_PN_SSG(args):
    ...
    if not args.evaluate_3d:
        ...
        pretrain_slip_model = torch.load('./data/initialize_models/slip_base_100ep.pt',
...
def ULIP2_PointBERT_Colored(args):
    ...
    ckpt_path = Path(__file__).resolve().parents[1] / "pretrained" / "open_clip_pytorch_model.bin"
