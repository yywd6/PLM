...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, return_layers=None):
...
class PointTransformer_Colored(nn.Module):
    ...
    def forward(self, pts, return_intermediate=False, return_layers=None):
                                                    
        neighborhood, center, patch_idx = self.group_divider(pts)
        group_input_tokens = self.encoder(neighborhood)         
        group_input_tokens = self.reduce_dim(group_input_tokens)
        ...
        if return_layers is None:
            ...
            x, features = self.blocks(x, pos, return_layers=return_layers)
        ...
        if return_intermediate:
            global_feat = x[:, 0]             
            local_feat = x[:, 1:]               
            return concat_f, global_feat, local_feat, features,neighborhood, center, patch_idx
        else:
            return concat_f
